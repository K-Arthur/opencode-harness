...
describe("EventNormalizer — behavioral", () => {
  ...
  it("normalizes v1 step-finish part to step_finish", () => {
    const events = normalizer.normalize({
      type: "message.part.updated",
      properties: {
        part: {
  ...
  it("normalizes v2 question lifecycle events", () => {
    const asked = normalizer.normalize({
      ...
      properties: {
        ...
        tool: { messageID: "m-question", callID: "call-question" },
      },
    })
    const replied = normalizer.normalize({
      type: "question.replied",
      properties: { sessionID: "s-question", requestID: "question-request-1", answers: [["Postgres"]] },
    })
    const rejected = normalizer.normalize({
      type: "question.rejected",
      properties: { sessionID: "s-question", requestID: "question-request-1" },
    })

    assert.deepEqual([...asked, ...replied, ...rejected].map((event) => event.type), [
      "question_asked",
      "question_replied",
      "question_rejected",
    ])
    assert.equal(asked[0]!.sessionId, "s-question")
    const data = asked[0]!.data as { requestID?: string; block?: { type?: string; toolCallId?: string; allowFreeText?: boolean } }
    assert.equal(data.requestID, "question-request-1")
    assert.equal(data.block?.type, "question")
    assert.equal(data.block?.toolCallId, "call-question")
  ...
  it("normalizes v2 permission.asked to permission_request", () => {
    ...
    assert.equal(events.length, 1)
    assert.equal(events[0]!.type, "permission_request")
    assert.equal(events[0]!.sessionId, "s-perm-v2")
    const data = events[0]!.data as { id?: string; type?: string; pattern?: string[] }
    assert.equal(data.id, "perm-v2")
    assert.equal(data.type, "edit")
    assert.deepEqual(data.pattern, ["src/**/*.ts"])
  })

  it("normalizes v2 session.next activity into visible stream events", () => {
    const events = [
      normalizer.normalize({
        type: "session.next.text.delta",
        properties: { sessionID: "s-next", delta: "hello" },
      }),
      normalizer.normalize({
        type: "session.next.reasoning.delta",
        properties: { sessionID: "s-next", reasoningID: "r1", delta: "thinking" },
      }),
      normalizer.normalize({
        type: "session.next.tool.called",
        properties: { sessionID: "s-next", callID: "tool-1", tool: "read", input: { file: "a.ts" } },
      }),
      normalizer.normalize({
        type: "session.next.tool.success",
        properties: { sessionID: "s-next", callID: "tool-1", structured: { ok: true }, content: [{ type: "text", text: "done" }] },
      }),
      normalizer.normalize({
    ...
    assert.equal((events[0]!.data as { text?: string }).text, "hello")
    assert.equal((events[1]!.data as { text?: string }).text, "thinking")
    assert.equal((events[2]!.data as { id?: string }).id, "tool-1")
  })

  it("classifies unknown event types instead of silently dropping them", () => {
    const events = normalizer.normalize({ type: "vendor.future.event", properties: { sessionID: "s-future" } })
    assert.equal(events.length, 1)
    assert.equal(events[0]!.type, "unknown_server_event")
    assert.equal(events[0]!.sessionId, "s-future")
  })

  it("returns empty array for unknown event types", () => {
    const events = normalizer.normalize({ type: "server.heartbeat" })
