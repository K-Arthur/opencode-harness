 ...
 * entry, merge the local transcript into the server-keyed entry and delete
 * the duplicate local key. This is non-destructive and prevents duplicate
 * history rows for the same server session.
 */
export function migrateLocalIdsToServerIds(
  ...
  for (const { from, to, sess } of targets) {
    const existing = bySessionId.get(to)
    if (existing) {
      mergeSessionIntoTarget(existing, sess)
      existing.id = to
      existing.cliSessionId = to
  ...
  for (const srv of serverSessions) {
    ...
    if (existing) {
      ...
      if (existing.id !== srv.id) {
        bySessionId.delete(existing.id)
        existing.id = srv.id
        bySessionId.set(srv.id, existing)
      }
