...
import { calculateProgress, applyTodoFilter, mergeTodos, generateTodoId, isUserTodo, type TodoSessionState } from "./todos-logic"
...
describe("todos-logic", () => {
  ...
  describe("isUserTodo", () => {
    it("returns true for todos with 'todo-' id prefix", () => {
      const t = makeTodo({ id: "todo-abc123" })
      assert.equal(isUserTodo(t), true)
    })

    it("returns false for server-v1 todos with explicit ids", () => {
      const t = makeTodo({ id: "server-todo-1" })
      assert.equal(isUserTodo(t), false)
    })

    it("returns false for synthesized v2 server todos with 'srv-' prefix", () => {
      const t = makeTodo({ id: "srv-1a2b3c-0" })
      assert.equal(isUserTodo(t), false)
    })

    it("returns false for todos with numeric or other id shapes", () => {
      assert.equal(isUserTodo(makeTodo({ id: "1" })), false)
      assert.equal(isUserTodo(makeTodo({ id: "abc-def-ghi" })), false)
    })
  })
})
