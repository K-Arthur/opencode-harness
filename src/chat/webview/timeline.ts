...
export function createTimeline(deps: TimelineDeps): TimelineAPI {
  ...
  function refreshConversationTimeline(sessionId?: string) {
    ...
    turns.forEach((turn, index) => {
      ...
      // Per-turn model indicator — compact CLI-style badge anchored below the
      // preview. Mirrors the message-header badge but sized for the narrow
      // sidebar. Hidden from keyboard tab stops (informational only); the full
      // model string is available via the native title tooltip.
      if (turn.model) {
        const modelBadge = document.createElement("span")
        modelBadge.className = "timeline-item-model"
        const modelShortName = turn.model.includes("/")
          ? turn.model.split("/").slice(1).join("/")
          : turn.model
        modelBadge.textContent = modelShortName
        modelBadge.title = `Generated by ${turn.model}`
        modelBadge.setAttribute("aria-label", `Generated by ${turn.model}`)
