...
type ToolStartPayload = { id: string; name: string; class?: string; args?: unknown; state?: ToolCallState; startedAt?: number; workingDir?: string }
...
function createToolCallBlock(toolCall: ToolStartPayload): ToolCallBlock {
  return {
    ...
    workingDir: toolCall.workingDir,
  }
}
...
export function handleToolUpdate(
  ...
  // Subagent cards have their own class taxonomy (.subagent-card--<status>
  // and .subagent-card-status) so the generic setToolStateClass / .tool-status
  // selectors below are no-ops on them. Route through the dedicated card
  // updater instead — otherwise the inline card stays stuck at its initial
  // render state until message_complete re-renders it.
  if (toolEl.classList.contains("subagent-card")) {
    applySubagentCardUpdate(toolEl, {
      state: update.state,
...
export function handleToolEnd(
  ...
  // Route to the subagent card updater for the same reason as
  // handleToolUpdate — generic class selectors don't match subagent cards.
  if (toolEl.classList.contains("subagent-card")) {
...
export function handleStreamError(
  ...
  const errorContext: ErrorContext = (() => {
    ...
    if (rawMsg && rawMsg.trimStart().startsWith("{")) {
      try {
        const parsed = JSON.parse(rawMsg) as Record<string, unknown>
        if (parsed && typeof parsed === "object") {
          const mapped = mapOpencodeError(parsed as Parameters<typeof mapOpencodeError>[0])
          if (mapped.userMessage && mapped.userMessage !== rawMsg) return mapped
        }
...
export function handleRunActivityUpdate(
  state: StreamState,
  els: StreamElements,
  activity: RunActivitySnapshot
): void {
  if (activity.phase === "completed" || activity.phase === "cancelled") {
    hideTypingIndicator(els)
    return
  }
  ...
  const label = activity.statusLabel || (
    activity.activeToolCount > 0
      ? "No new text yet, tools are still active"
      : activity.activeSubagentCount > 0
        ? `Reviewing with ${activity.activeSubagentCount} active subagent${activity.activeSubagentCount === 1 ? "" : "s"}`
        : "Still working"
  )
  showTypingIndicator(els, label)
}
