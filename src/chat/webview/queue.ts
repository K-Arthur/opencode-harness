...
export interface PromptQueue {
  ...
  /** Replace all items with host-provided data. Used to sync host queue state into the webview render cache. */
  syncFromHost: (items: QueueItem[]) => void
}
...
export function createPromptQueue(): PromptQueue {
  ...
  function syncFromHost(hostItems: QueueItem[]): void {
    items.length = 0
    for (const hostItem of hostItems) {
      const item: QueueItem = {
        id: hostItem.id,
        text: hostItem.text,
        attachments: hostItem.attachments || [],
        state: hostItem.state,
        createdAt: hostItem.createdAt || Date.now(),
        error: hostItem.error,
        position: hostItem.position ?? items.length,
        isSteerPrompt: hostItem.isSteerPrompt,
        estimatedTokens: hostItem.estimatedTokens ?? estimateTextTokens(hostItem.text),
      }
      items.push(item)
    }
  }
