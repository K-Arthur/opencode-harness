...
export function setupVoiceInput(deps: VoiceInputDeps): VoiceInputApi {
  ...
  function syncButton(): void {
    ...
    // are interactive.
    const canOpenSetup = state === "disabled" && settings?.enabled !== false && settings?.available === false
    els.voiceInputBtn.disabled = !canOpenSetup && (state === "disabled" || state === "transcribing")
    if (canOpenSetup) {
      els.voiceInputBtn.title = "Set up voice input"
      els.voiceInputBtn.setAttribute("aria-label", "Set up voice input")
      return
    }
  ...
  function onClick(): void {
    if (state === "disabled" && settings?.enabled !== false && settings?.available === false) {
      postMessage({ type: "setup_voice_input" })
      return
    }
