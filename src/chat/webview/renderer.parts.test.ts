...
describe("renderer — Layer 7 RED (canonical part renderers)", () => {
  ...
  it("L7-T9: file renders as an inline image or attachment chip", () => {
    assertRenderer("file", "renderFileBlock", "L7-T9")
    const body = source.slice(source.indexOf("function renderFileBlock"))
    assert.match(body, /mime\.startsWith\("image\/"\)/, "file renderer must detect image MIME types")
    assert.match(body, /msg-file-chip/, "file renderer must fall back to a visible attachment chip")
  })
