...
describe("T1.1 — TTFB timeout cancels in-flight request", () => {
  it("creates an AbortController per tab before calling sendPromptAsync", () => {
    ...
    assert.ok(timeoutManagerSource.includes("ttfbAbortControllers.set(tabId, abortController)"), "must store abort controller per tab")
  })
  ...
  it("sendPromptAsync accepts signal in PromptOptions", () => {
    assert.ok(sessionTypesSource.includes("signal?: AbortSignal"), "PromptOptions must have signal property")
  })
