...
test.describe('Chat Messages', () => {
  ...
  test('assistant tool calls render as individual rows when text intervenes', async ({ page }) => {
    ...
    // Grouping is CONSECUTIVE-only: visible text between tool calls breaks
    // the group (see groupConsecutiveToolCalls — "DOES break the group on a
    // visible text block"). Each tool renders as its own details row.
    ...
    await expect(page.locator('.message.assistant details.tool-group')).toHaveCount(0)
  })
