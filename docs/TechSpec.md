...
- **TTFB timeout**: `TTFB_TIMEOUT_MS = 90000` (first byte, extended from 45s for slow third-party model providers). TTFB timer is cleared on first chunk. The separate chunk-inactivity timeout was removed; `STREAM_STUCK_MS` (45min) serves as the single hard-cap inactivity watchdog.
